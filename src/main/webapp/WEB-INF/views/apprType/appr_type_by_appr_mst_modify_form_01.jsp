    ...
    $(function(){
	    ...
	    $('#btnSave').bind('click', function(){
	    	if($('#apprTypeByApprMstAddForm').form('enableValidation').form('validate')) {
	    		...
	    		var strUrl = '<c:out value="${contextPath}"/>/rest/apprMstByApprovalType/modify';	    		    		
	...
	<jsp:include page="/WEB-INF/views/include/hidden_type_01.jsp"></jsp:include>
		<table style="width:100%">
	        <tr>
	        	<td style="width:50%;padding:0px 10px;">			        	
		            <div style="margin-bottom:10px">
		                <input class="easyui-textbox" id="coId" name="coId" style="width:100%" value="${nw015m.coId}" data-options="label:'<spring:message code="resc.label.coId"/>:',readonly:true,required:true,labelWidth:120">
		            </div>
	            </td>
	            <td style="width:50%;padding:0px 10px;">		        	
		            <div style="margin-bottom:10px">		            	
		                <select class="easyui-combobox" id="apprMstId" name="apprMstId" style="width:100%;" data-options="panelHeight:'auto',label:'<spring:message code="resc.label.apprMasterId"/>:',readonly:true,required:true,labelWidth:120">
		                <c:forEach items="${apprMstList}" var="apprMst">
		                	<option value="${apprMst.apprMstId}" <c:if test="${apprMst.apprMstId == nw015m.apprMstId}">selected="selected"</c:if> >${apprMst.apprMstId}:${apprMst.apprDesc}</option>
		                </c:forEach>
		                </select>		                		                
		            </div>
	            </td>	            
	        ...
	        <tr>
	        	<td style="width:50%;padding:0px 10px;">
		            <div style="margin-bottom:10px">
		                <input class="easyui-textbox" id="apprItemId" name="apprItemId" style="width:100%" value="${nw015m.apprItemId}" data-options="label:'<spring:message code="resc.label.apprType"/>:',readonly:true,required:true,labelWidth:120">
		            </div>
	            </td>
	            <td style="width:50%;padding:0px 10px;">	            	
		            <div style="margin-bottom:10px">		            		    
		                <input class="easyui-textbox" id="seqNo" name="seqNo" style="width:100%" value="${nw015m.seqNo}" data-options="label:'<spring:message code="resc.label.seqNo"/>:',required:true,labelWidth:120">		                
		            </div>
	            </td>
	        </tr>	        
        </table>
