...
<title><spring:message code="resc.menu.contentsMgr"/></title>
    ...
    $(function(){
    	$("#cntnList").datagrid({
    		onClickRow: function() {
    			var rowData = $("#cntnList").datagrid('getSelected');
    			
    			strCntnId = rowData.cntnId;    				
    		}
    	});
    	
    	$('#btnSelect').bind('click', function(){
    		var typeCodeName = '${typeCodeName}';
    		window.opener.$('#' + typeCodeName).textbox('setValue', strCntnId);
    		window.close();
    	});
    });
...
<body>
	...
	<table style="width:100%">		
		<tr>
			<td style="width:50%">				
       			<table id="cntnList" class="easyui-datagrid" style="width:100%;height:390px;"		        
			       		title="<spring:message code="resc.label.masterList"/>" 
			       		data-options="rownumbers:true, singleSelect:true, collapsible:false,url:'<c:out value="${contextPath}"/>/rest/contents/coId/${userSession.coId}/searchKind/cntnId/searchWord/0/orderBy/cntnId', method:'get', toolbar:'#masterCodeTb', pagination:false, autoRowHeight:false">
			        <thead>
			            <tr>
			                <th data-options="field:'coId',halign:'center',align:'center',width:'20%'"><spring:message code="resc.label.coId"/></th>
			                <th data-options="field:'cntnId',halign:'center',align:'center',width:'30%'"><spring:message code="resc.label.masterId"/></th>			                
			                <th data-options="field:'cntnName',halign:'center',align:'left',width:'50%'"><spring:message code="resc.label.masterName"/></th>
			            </tr>
			        </thead>
			    </table>    				   
			</td>
